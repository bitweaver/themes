...
deprecated
	function tc_assign_section($section, $theme) {
		...
		$this->query($query,array($section),-1,-1);
...
*/
	function tc_assign_object($obj_id, $theme, $type, $name) {
		...
		$this->query($query,array($obj_id),-1,-1);
...
/*
	function tc_get_theme_by_section($section) {
		if ($this->getOne("select count(*) from `".BIT_DB_PREFIX."tiki_theme_control_sections` where `section`=?",array($section))) {
			return $this->getOne("select `theme` from `".BIT_DB_PREFIX."tiki_theme_control_sections` where `section`=?",array($section));
		} else {
			return '';
		}
	}
*/
	function tc_get_theme_by_object($type, $obj_id) {
		...
		if ($this->getOne("select count(*) from `".BIT_DB_PREFIX."tiki_theme_control_objects` where `type`=? and `obj_id`=?",array($type,$obj_id))) {
			return $this->getOne("select `theme` from `".BIT_DB_PREFIX."tiki_theme_control_objects` where `type`=? and `obj_id`=?",array($type,$obj_id));
		} else {
			return '';
		}
	}
	...
	function tc_list_categories($offset, $maxRecords, $sort_mode, $find) {
		...
		$query = "select tc.`category_id`,tc.`name`,`theme` from `".BIT_DB_PREFIX."tiki_theme_control_categs` ttt,`".BIT_DB_PREFIX."tiki_categories` tc where ttt.`category_id`=tc.`category_id` $mid order by ".$this->convert_sortmode($sort_mode);
...
/*
	function tc_list_sections($offset, $maxRecords, $sort_mode, $find) {
		...
		$query = "select * from `".BIT_DB_PREFIX."tiki_theme_control_sections` $mid order by ".$this->convert_sortmode($sort_mode);
...
*/
	function tc_list_objects($type, $offset, $maxRecords, $sort_mode, $find) {
		...
		$query = "select * from `".BIT_DB_PREFIX."tiki_theme_control_objects` where `type`=? $mid order by ".$this->convert_sortmode($sort_mode);
