...
class TemplatesLib extends BitBase {
	...
	function list_all_templates($offset, $maxRecords, $sort_mode, $find) {
		...
		$query = "select `name`,`created`,`template_id` from `".BIT_DB_PREFIX."tiki_content_templates` $mid order by ".$this->convert_sortmode($sort_mode);
	...
	function replace_template($template_id, $name, $content) {
		...
		if ($template_id) {
			...
			$this->query($query,array($content,$name),-1,-1);
			$query = "insert into `".BIT_DB_PREFIX."tiki_content_templates`(`content`,`name`,`created`) values(?,?,?)";
		}

		$result = $this->query($query,$bindvars);
		$id = $this->getOne("select max(`template_id`) from `".BIT_DB_PREFIX."tiki_content_templates` where `created`=? and `name`=?",array((int)$now,$name));
		return $id;
	}

	function add_template_to_section($template_id, $section) {
		$this->query("delete from `".BIT_DB_PREFIX."tiki_content_templates_sections` where `template_id`=? and `section`=?",array((int)$template_id,$section),-1,-1);
		$query = "insert into `".BIT_DB_PREFIX."tiki_content_templates_sections`(`template_id`,`section`) values(?,?)";
		$result = $this->query($query,array((int)$template_id,$section));
	}

	function remove_template_from_section($template_id, $section) {
		$result = $this->query("delete from `".BIT_DB_PREFIX."tiki_content_templates_sections` where `template_id`=? and `section`=?",array((int)$template_id,$section));
	}

	function template_is_in_section($template_id, $section) {
		$cant = $this->getOne("select count(*) from `".BIT_DB_PREFIX."tiki_content_templates_sections` where `template_id`=? and `section`=?",array((int)$template_id,$section));
		return $cant;
	}
